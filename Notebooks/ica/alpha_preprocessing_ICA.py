...
sourcedata_dir = '/Users/juliette/Desktop/Radboud/work/BCI/cVEP'
save_dir = '/Users/juliette/Desktop/thesis/preprocessing/alpha_preprocessing/alpha_ICA'
ica_dir = '/Users/juliette/Desktop/thesis/ica'
...
V = np.load('/Users/juliette/Desktop/thesis/mgold_61_6521.npz')["codes"]
...
file_path = '/Users/juliette/Desktop/thesis/ica'
subjects_channel_reject = {}

loc_file = '/Users/juliette/Desktop/thesis/biosemi64.loc'
montage = mne.channels.read_custom_montage(loc_file)

for sub in subjects:
    fn = os.path.join('/Users/juliette/Desktop/thesis/ica', f"sub-{sub}", f"sub-{sub}_bad_elec.csv")

    if not os.path.exists(fn):  # Check if the file exists
        print(f"Warning: {fn} not found")
        subjects_channel_reject[sub] = []  # Store an empty list if the file is missing
        continue

    df = pd.read_csv(fn)
    subjects_channel_reject[sub] = df.iloc[:, 0].dropna().tolist()  # Drop NaN values and store
...
for i_subject, subject in enumerate(subjects):
    ...
    # Load ICA solution
    ica_path = os.path.join(ica_dir, f"sub-{subject}", f"{subject}_ICA_.joblib")
    ica = joblib.load(ica_path)

    # Load the rejected ICA components from CSV
    deleted_components_path = os.path.join(ica_dir, f"sub-{subject}", f"sub-{subject}_deleted_eye_components.csv")
    ...
    for i_task, task in enumerate(tasks):
        ...
        for i_run, run in enumerate(runs[task]):
            ...
            # Apply ICA to remove artifacts, before epoching
            ica.apply(raw)

            # Apply Hübner electrode selection
            available_hubner = [ch for ch in picks_hubner if ch in raw.info['ch_names']]
            raw.pick(available_hubner)
        ...
        np.savez(os.path.join(save_dir, f"sub-{subject}_task-{task}_alpha_ICA.npz"), X=X, y=y, z=z, V=V, fs=fs)
